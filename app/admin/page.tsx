 ...
 * admin-page.tsx
 * ─────────────────────────────────────────────────────────────
 * Panel de administración de Radar.
 * Solo accesible para usuarios con role === "admin".
 *
 * Secciones:
 *   1. Estadísticas generales (tarjetas)
 *   2. Usuarios registrados (tabla con acciones)
 *   3. Suscripciones a newsletter
 *   4. Actividad reciente (mock)
 * ─────────────────────────────────────────────────────────────
 */
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-slate-700/40">
              {["Usuario", "Email", "Rol", "Registrado", "Acciones"].map((h) => (
                <th key={h} className="text-left text-[10px] font-bold text-slate-500 uppercase tracking-widest px-5 py-3">
                  {h}
                </th>
              ))}
            </tr>
          ...
          <tbody>
            {filtered.length === 0 && (
              <tr>
                <td colSpan={5} className="text-center py-10 text-slate-600">No hay usuarios que coincidan.</td>
              </tr>
            )}
            {filtered.map((u) => (
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {confirmDelete === u.id ? (
                        <div className="flex items-center gap-1.5">
                          ...
                          <button onClick={() => setConfirmDelete(null)} className="text-[11px] text-slate-500 hover:text-slate-300 transition-colors">
                            Cancel
                          </button>
                        </div>
                      ) : (
  ...
  return (
    <div className="bg-slate-800/50 border border-slate-700/60 rounded-2xl p-5">
      <h3 className="font-bold text-white mb-4 flex items-center gap-2">
        <span>📬</span> Suscripciones a Newsletter
      </h3>
    ...
    <Shell>
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        {activeTab === "overview" && stats && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <StatCard value="12" label="Fuentes activas" icon="📡" color="text-[#3EB2ED]" />
        ...
        {activeTab === "users" && stats && (
          <UsersTable
            users={stats.recentUsers.length > 0
              ? stats.recentUsers
              : []}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="bg-slate-800/50 border border-slate-700/60 rounded-2xl p-5">
              ...
              <div className="flex flex-col items-center justify-center py-10 gap-3 text-slate-600">
                ...
                <p className="text-xs">Se integrarán con el sistema de envíos.</p>
              </div>
            </div>
          </div>
        )}
